        ...
        <div class="chat-dialog" id="chatDialog">
            <div class="chat-header">
                ...
                <div class="chat-header-buttons">
                    <button type="button" class="btn btn-outline-light btn-sm me-2" id="newSessionButton"
        ...
        <script>
            document.addEventListener('DOMContentLoaded', function () {
                ...
                newSessionButton.addEventListener('click', () => {
                    if (confirm(
                        'Are you sure you want to start a new chat session? This will clear the current conversation.'
                    )) {
                        startNewSession();
                    }
                });
                ...
                function startNewSession() {
                    // Clear chat messages immediately for better UX
                    chatMessages.innerHTML = '';

                    // Clear chat history on server using the dedicated endpoint
                    fetch(generateUrl({ module: 'chat', action: 'start_new_chat' }), {
                        method: 'POST',
                        headers: {
                            'Content-Type': 'application/json',
                        }
                        ...
                        .then(data => {
                            if (data.success) {
                                ...
                                console.error('Failed to start new chat session:', data.error);
                                // Still add welcome message even if server clear failed
                                addMessage('Hello! How can I help you with your shoe shopping today?', 'bot');
                            }
                        })
                        .catch(error => {
                            console.error('Error starting new chat session:', error);
                            // Still add welcome message even if request failed
                            addMessage('Hello! How can I help you with your shoe shopping today?', 'bot');
                        });
                }
